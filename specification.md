...
- 欄位如果是文字，要協助做前後trim(大寫空白亦要消掉)

## 7. 固定輸出欄位結構

轉換後每筆 SQL 的欄位順序固定如下：

`ACCTNO,UID,PWDHASHCODE,FORCEUPD,LOCKNUM,USERNAME,BIRTHDATE,EMAIL,MOBILE,ZIPCODE,ADDRESS_3,AGREE_SALES,ACCT_STATE,CRT_DATE,UPD_DATE,GENDER`

轉換後每筆 SQL 的前綴格式為（table 名稱由 `config.json` 的 `table_name` 決定）：

```sql
INSERT INTO {table_name} (ACCTNO,UID,PWDHASHCODE,FORCEUPD,LOCKNUM,USERNAME,BIRTHDATE,EMAIL,MOBILE,ZIPCODE,ADDRESS_3,AGREE_SALES,ACCT_STATE,CRT_DATE,UPD_DATE,GENDER)
```
...
`ACCTNO,PWDHASHCODE,FORCEUPD,LOCKNUM,USERNAME,BIRTHDATE,EMAIL,MOBILE,ZIPCODE,ZIPCODE_ORIGIN,ADDRESS_3,ADDRESS_3_ORIGIN,AGREE_SALES,ACCT_STATE,CRT_DATE,UPD_DATE,GENDER,COMENTS`
...
- 若 `ZIPCODE` 為 SQL `null` 或 `NULL`，則：
  - 抓取 `ADDRESS_3` 第一至第三字元與 zipcode.json `city` 比對、第四至第六字元與 `name` 比對：
  - TRUE（比對成功）：
    - COPY zipCode of zipcode.json to `ZIPCODE`
    - COPY `null` to `ZIPCODE_ORIGIN`
  - FALSE（比對失敗）：
    - `ZIPCODE` 設為空字串 `''`
    - COPY `null` to `ZIPCODE_ORIGIN`
  - CSV `COMENTS` 欄位新增文字 `"客戶資料無zipcode"`

### 10.2 ZIPCODE 不為 null，且查無對應
...
- 若 `GENDER` 為 `F`、`f`、或空字串，轉為 `'0'`
- 若 `GENDER` 為 `null`，轉為 `'0'`
- 其他非預期值轉為 `'9'`，並在 CSV COMENTS 欄位新增文字"客戶性別資料不正確"
